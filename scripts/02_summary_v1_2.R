...
suppressPackageStartupMessages({
  ...
  # sort out for long subheadings to plots
  if (!requireNamespace("stringr", quietly = TRUE)) install.packages("stringr")
  
  pad_titles <- function(p, top = 16, right = 12, bottom = 12, left = 12, wrap = 90) {
    # wrap long subtitles/titles to avoid clipping on save
...
# ---- Write per-day counts to CSVs ----
readr::write_csv(day_counts,
                 file.path(DIR_OUTPUTS, glue::glue("day_counts__{tag}.csv")))
...
readr::write_csv(day_counts_species,
                 file.path(DIR_OUTPUTS, glue::glue("day_counts_by_species__{tag}.csv")))
...
show_and_save(p_gam_wt, glue("motility_prop_over_time_GAM_weighted__{tag}.png"),
              width = 180, height = 140)       # a touch taller helps
export_figure(p_gam_wt, glue("motility_prop_over_time_GAM_weighted__{tag}.png"),
