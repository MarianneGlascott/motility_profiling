...
# 6. Run BOTH analyses
# 6b. By-species overlays and 2×2 panel (+ per-species saves)
...
#      Paste AFTER Section 6; requires 'd' in memory
# ==============================================================================
dplyr::count(d, species, sort = TRUE) |> print(n = Inf)

# Canonicalise species names (defensive)
...
d <- d |> dplyr::mutate(species = canonicalise_species(species))
dplyr::count(d, species, sort = TRUE) |> print(n = Inf)
...
make_sp_plot <- function(df_w, sp_name) {
  ...
  pct    <- if (is.finite(m0) && m0 > 0) 100 * (mL - m0) / m0 else NA_real_
  annot  <- glue::glue("Weighted GAM (n/day); Linear slope {sprintf('%.3f', slope)}/day ",
  ...
  ggplot(dd_w, aes(days_from_start, motility_prop)) +
    ...
    scale_y_continuous(limits = c(0, 1)) +
    labs(title = sp_name, subtitle = annot,
         x = "Days from start", y = "Motility proportion")
}
...
# Save each species plot separately
to_file_stub <- function(sp) {
  out <- gsub("[^A-Za-z0-9]+", "_", sp, perl = TRUE)
  tolower(out)
}
plots_list <- list(
  "L. digitata"   = p_ld,
...
# ---------- Overlay: overall + per-species + overall (with CI) ----------------
suppressPackageStartupMessages({ library(ggplot2); library(dplyr); library(mgcv) })

# Keep only species with enough distinct days to fit a smooth
valid_species <- sp_day_counts |>
  dplyr::group_by(species) |>
  dplyr::summarise(n_days = dplyr::n_distinct(days_from_start), .groups = "drop") |>
  dplyr::filter(n_days >= 3) |>
  dplyr::pull(species)

d_w_sp_valid <- d_w_sp |> dplyr::filter(species %in% valid_species)

# Explicit colours (so it’s definitely not greyscale)
sp_cols <- c(
  "L. digitata"   = "#1f77b4",  # blue
  "L. hyperborea" = "#2ca02c",  # green
  "S. latissima"  = "#17becf"   # teal
)

# Overall slope + % change for subtitle (robust, no inline if)
lm_all  <- lm(motility_prop ~ days_from_start, data = d)
s_all   <- unname(coef(lm_all)[["days_from_start"]])
ci_all  <- unname(confint(lm_all)["days_from_start", ])

d0_all  <- suppressWarnings(min(d$days_from_start, na.rm = TRUE))
dL_all  <- suppressWarnings(max(d$days_from_start, na.rm = TRUE))
m0_all  <- suppressWarnings(mean(d$motility_prop[d$days_from_start == d0_all], na.rm = TRUE))
mL_all  <- suppressWarnings(mean(d$motility_prop[d$days_from_start == dL_all], na.rm = TRUE))

pct_note <- ""
if (is.finite(m0_all) && m0_all > 0 && is.finite(mL_all)) {
  pct_note <- sprintf("; Δ%% %d→%d: %.1f%%", d0_all, dL_all, 100 * (mL_all - m0_all) / m0_all)
}
annot_all <- paste0(
  "Coloured lines: species-weighted GAMs (n/day/species); ",
  "black line: overall weighted GAM (n/day) with 95% CI. ",
  sprintf("Overall slope %.3f/day [%.3f, %.3f]", s_all, ci_all[1], ci_all[2]),
  pct_note
)

# Plot
p_overlay_all_vs_species <- ggplot() +
  # faint raw points per species
  geom_point(
    data = d_w_sp_valid,
    aes(days_from_start, motility_prop, colour = species),
    alpha = 0.18, size = 1.2
  ) +
  # species GAMs (no CI to keep clean)
  geom_smooth(
    data = d_w_sp_valid,
    aes(days_from_start, motility_prop, colour = species, weight = wt),
    method = "gam", formula = y ~ s(x, k = 6), se = FALSE, linewidth = 1.0
  ) +
  # overall weighted GAM with CI (black)
  geom_smooth(
    data = d_w_all,
    aes(days_from_start, motility_prop, weight = wt),
    method = "gam", formula = y ~ s(x, k = 6), se = TRUE,
    colour = "black", linewidth = 1.2
  ) +
  scale_x_continuous(breaks = sort(unique(d$days_from_start))) +
  scale_y_continuous(limits = c(0, 1)) +
  scale_colour_manual(values = sp_cols, drop = FALSE) +
  guides(colour = guide_legend(override.aes = list(linewidth = 1.5, alpha = 1))) +
  labs(
    title = "Motility proportion over time — Overall and by species (overlay)",
    subtitle = annot_all,
    x = "Days from start", y = "Motility proportion", colour = "Species"
  ) +
  theme(legend.position = "bottom")

if (exists("pad_titles")) p_overlay_all_vs_species <- pad_titles(p_overlay_all_vs_species, top = 18, wrap = 88)

print(p_overlay_all_vs_species)
show_and_save(
  p_overlay_all_vs_species,
  "motility_prop_over_time_GAM_weighted__OVERLAY_all_plus_species.png",
