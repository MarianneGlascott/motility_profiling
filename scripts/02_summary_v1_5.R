...
run_summary <- function(data, species_filter = NA_character_, label = "all_species") {
  ...
  # Per-day counts (for figures and small-n logs)
  day_counts <- dd |>
    dplyr::group_by(days_from_start) |>
    dplyr::summarise(
  ...
  # ---------------------------------------------------------------------------
  # B1. Trend modelling: linear decline and test for non-linearity
  # ---------------------------------------------------------------------------
  
  # Linear model for effect size (reported slope)
  ...
  # GAM to test for non-linearity (not for slope estimation)
  gam_fit <- mgcv::gam(
  ...
  # Extract GAM smooth statistics for reporting
  ...
  readr::write_csv(
    gam_tbl,
    file.path(DIR_OUTPUTS, glue("gam_time_smooth_results__{tag}.csv"))
  )
  
  
  # ---------------------------------------------------------------------------
  # B2. Visualisation: weighted GAM smooth with effect-size annotation
  # ---------------------------------------------------------------------------
  ...
  # Keep a lightweight trend table for small-n checks
