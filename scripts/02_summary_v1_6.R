...
#   Species views beyond ALL vs L. digitata:
#     E) Overlay: overall curve + coloured species curves (with CI on overall)
#     F) 2×2 panel: All + each species (and save each species plot individually)
...
suppressPackageStartupMessages({
  ...
  library(patchwork)    # side-by-side composition/panels
...
# ==============================================================================
# 2. Read data, coerce types, canonicalise species
# ==============================================================================
...
d <- d0 |>
  mutate(
    ...
    species              = as.character(species),  # canonicalise on character, then factor
...
# ---- Canonicalise species labeling early (pre-analysis) -----------------------
canonicalise_species <- function(x) {
  x0 <- stringr::str_squish(stringr::str_trim(as.character(x)))
  lab <- dplyr::case_when(
    stringr::str_detect(stringr::str_to_lower(x0), "digitata")   ~ "L. digitata",
    stringr::str_detect(stringr::str_to_lower(x0), "hyperborea") ~ "L. hyperborea",
    stringr::str_detect(stringr::str_to_lower(x0), "latissima")  ~ "S. latissima",
    TRUE ~ x0
  )
  factor(lab, levels = c("L. digitata", "L. hyperborea", "S. latissima"))
}
d <- d |> mutate(species = canonicalise_species(species))
...
run_summary <- function(data, species_filter = NA_character_, label = "all_species") {
  ...
  # ---------------------------------------------------------------------------
  # 5.2 Overall distributions
  # ---------------------------------------------------------------------------
  overall_tbl <- purrr::map_dfr(vars_to_summarise, function(v) {
  ...
  # ---------------------------------------------------------------------------
  # 5.3 Snapshot summaries (focus days: 0/1/4)
  # ---------------------------------------------------------------------------
  byday_tbl <- dd |>
    dplyr::filter(days_from_start %in% FOCUS_DAYS) |>
    dplyr::group_by(days_from_start) |>
    dplyr::summarise(
      dplyr::across(
        dplyr::any_of(vars_to_summarise),
        list(
          n      = ~sum(is.finite(.x)),
          mean   = ~ifelse(sum(is.finite(.x)) > 0, mean(.x, na.rm = TRUE), NA_real_),
          se     = ~{
            n <- sum(is.finite(.x))
            if (n > 1) stats::sd(.x, na.rm = TRUE) / sqrt(n) else NA_real_
          },
          median = ~ifelse(sum(is.finite(.x)) > 0, median(.x, na.rm = TRUE), NA_real_),
          q1     = ~ifelse(sum(is.finite(.x)) > 0, quantile(.x, 0.25, na.rm = TRUE), NA_real_),
          q3     = ~ifelse(sum(is.finite(.x)) > 0, quantile(.x, 0.75, na.rm = TRUE), NA_real_)
        ),
        .names = "{.col}.{.fn}"
      ),
      .groups = "drop"
  ...
  if (nrow(byday_tbl) == 0) stop("byday_tbl is empty — check FOCUS_DAYS and dd coverage.")
  readr::write_csv(byday_tbl, file.path(DIR_OUTPUTS, glue("byday_summary_0_1_4__{tag}.csv")))
  
  # Decline 0 → 4 (mean & median)
  d0row <- byday_tbl |> dplyr::filter(days_from_start == 0)
  d4row <- byday_tbl |> dplyr::filter(days_from_start == 4)
  
  decline_tbl <- tibble::tibble(
    group = tag,
  ...
  # ---------------------------------------------------------------------------
  # 5.4 Per-day counts (used for labels and weights)
  # ---------------------------------------------------------------------------
  day_counts <- dd |>
    dplyr::group_by(days_from_start) |>
    dplyr::summarise(
      ...
      se   = {
        nn <- sum(is.finite(motility_prop))
        if (nn > 1) stats::sd(motility_prop, na.rm = TRUE) / sqrt(nn) else NA_real_
      },
      .groups = "drop"
    ) |>
    dplyr::filter(n > 0)
  ...
  # Build weighted dataset (adds wt column)
  dd_w <- dd |>
    dplyr::left_join(
      day_counts |>
  ...
  if (!("wt" %in% names(dd_w))) stop("Weights column 'wt' was not created in dd_w.")
  
  # ---------------------------------------------------------------------------
  # 5.5 Plot A: dot-swarm + mean ± SE + n labels
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # 5.6 Trend modelling: linear effect size + GAM non-linearity test
  # ---------------------------------------------------------------------------
  # Linear model for effect size (slope per day)
  ...
  # GAM for curvature check (use gaussian on proportion for robustness)
  gam_fit <- mgcv::gam(
    motility_prop ~ s(days_from_start, k = 6),
    data    = dd_w,
    weights = wt,
    method  = "REML"
  )
  ...
  gam_tbl <- tibble::tibble(
    term = rownames(gam_sum$s.table),
    edf  = gam_sum$s.table[, "edf"],
    F    = gam_sum$s.table[, "F"],
    p    = gam_sum$s.table[, "p-value"]
  )
  
  readr::write_csv(gam_tbl, file.path(DIR_OUTPUTS, glue("gam_time_smooth_results__{tag}.csv")))
  
  # ---------------------------------------------------------------------------
  # 5.7 Plot B: weighted GAM smooth + effect-size subtitle
  # ---------------------------------------------------------------------------
  annot <- glue::glue(
  ...
  # Panel (A + B)
  p_panel <- p_swarm + p_gam_wt + patchwork::plot_layout(widths = c(1, 1)) &
  ...
  show_and_save(p_panel, glue("motility_over_time_panel_swarm_plus_smooth__{tag}.png"), width = 220, height = 140)
  export_figure(p_panel, glue("motility_over_time_panel_swarm_plus_smooth__{tag}.png"), width = 220, height = 140)
  
  # ---------------------------------------------------------------------------
  # 5.8 Species snapshot (days 0/1/4) — only for ALL run
  # ---------------------------------------------------------------------------
  if (is.na(species_filter)) {
    trend_sp <- dd |>
      ...
      dplyr::filter(days_from_start %in% FOCUS_DAYS) |>
      dplyr::mutate(n_flag = ifelse(n < QC_SMALL_N, " (n<15)", ""))
    ...
    p_trend_sp <- ggplot(trend_sp, aes(days_from_start, mean, colour = species, group = species)) +
      ...
      geom_point() +
      labs(
        title = "Motility proportion by species (days 0/1/4)",
  ...
  # ---------------------------------------------------------------------------
  # 5.9 Intro paragraph (draft)
  # ---------------------------------------------------------------------------
  compose_intro_paragraph <- function(overall_tbl, byday_tbl, dd_label) {
    fmtp <- function(x) scales::percent(x, accuracy = 0.1)
    m_med <- overall_tbl |> dplyr::filter(variable == "motility_prop") |> dplyr::pull(median)
    m_q1  <- overall_tbl |> dplyr::filter(variable == "motility_prop") |> dplyr::pull(q1)
    m_q3  <- overall_tbl |> dplyr::filter(variable == "motility_prop") |> dplyr::pull(q3)
    s_med <- overall_tbl |> dplyr::filter(variable == "settlement_count") |> dplyr::pull(median)
  ...
  intro_text <- compose_intro_paragraph(
    overall_tbl, byday_tbl,
    ifelse(is.na(species_filter), "across three species", paste0("in ", species_filter))
  )
  writeLines(intro_text, file.path(DIR_OUTPUTS, glue("intro_paragraph_draft__{tag}.txt")))
  
  # ---------------------------------------------------------------------------
  # 5.10 Return key outputs
  # ---------------------------------------------------------------------------
  return(list(
    tag     = tag,
    n       = nrow(dd),
...
# outputs for GAM (include in manuscript) for check for non-linearity
res_all <- run_summary(d)
res_all$gam_tbl
